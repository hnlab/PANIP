...
```bash
python random_select_frames.py --indir PANIP/examples/ --percent 2 --xyzdir PANIP/examples/data_pool/ --init
```
...
Typically, three models are trained in parallel. The prediction results of different models should be stored in separate directories.
```bash
# You can also use --xyzfile to batch submit prediction jobs on cluster
python evaluate_bad_data.py --indexdir PANIP/examples/ --model /path/of/model/ --xyzdir PANIP/examples/data_pool/ --output PANIP/examples/pred/
```
...
```bash
python random_select_frames.py --indir PANIP/examples/ --percent 5 --predir PANIP/examples/pred --xyzdir PANIP/examples/data_pool
```
If you want to judge prediction quality based on the average error across multiple parallel models, you can provide multiple prediction directories after `--predir`.  
